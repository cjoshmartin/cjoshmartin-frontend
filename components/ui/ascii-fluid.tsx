...
export function createAsciiFluid(
  ...
  const tick = (now: number) => {
    ...
    // Splat distance is measured in units of (screen px / canvas height), so
    // a fixed radius constant shrinks to almost nothing in a short container
    // (e.g. a navbar). Target a consistent on-screen radius instead.
    const targetRadiusPx =
      16 + Math.max(0.05, Math.min(1, p.brush)) * 40
    const brushR = Math.pow(targetRadiusPx / Math.max(canvas.height, 1), 2)
    ...
    if (p.animate && !reduce && !mouse.inside) {
      const swirlX = 0.5 + Math.sin(time * 0.23) * 0.22
      const swirlY = 0.5 + Math.cos(time * 0.19) * 0.18
      const swirlR = brushR * 3
      ...
      gl.bindTexture(gl.TEXTURE_2D, velocity.read.tex)
      gl.uniform1i(gl.getUniformLocation(splat.program, "u_target"), 0)
      gl.uniform2f(gl.getUniformLocation(splat.program, "u_point"), swirlX, swirlY)
      gl.uniform3f(gl.getUniformLocation(splat.program, "u_color"), ax, ay, 0)
      gl.uniform1f(gl.getUniformLocation(splat.program, "u_radius"), swirlR)
      gl.uniform1f(gl.getUniformLocation(splat.program, "u_aspect"), aspect)
      gl.uniform1f(gl.getUniformLocation(splat.program, "u_velocityField"), 1)
      blit(velocity.write)
      velocity.swap()

      // Without this, "animate" only stirs an invisible velocity field —
      // there's no dye to advect, so idle state never actually shows anything.
      gl.activeTexture(gl.TEXTURE0)
      gl.bindTexture(gl.TEXTURE_2D, dye.read.tex)
      gl.uniform1i(gl.getUniformLocation(splat.program, "u_target"), 0)
      gl.uniform2f(gl.getUniformLocation(splat.program, "u_point"), swirlX, swirlY)
